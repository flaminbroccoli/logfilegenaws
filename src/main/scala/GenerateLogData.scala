...
//this is the main starting point for the log generator
  def main(args:Array[String]):Unit =
    ...
    val s3 = AmazonS3ClientBuilder.standard().withRegion(Parameters.bucket_region).build()
    val xfer_mgr: TransferManager = TransferManagerBuilder.standard.withS3Client(s3).build()
    val xfer = xfer_mgr.uploadDirectory(Parameters.bucket_name,
      "log", new File(Parameters.log_path), true);
    logger.info("Awaiting upload completion")
